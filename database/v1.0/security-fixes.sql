...
-- ============================================================================
-- This fixes the Supabase database linter warning:
-- "function_search_path_mutable" - 8 functions affected
-- 
-- Severity: WARN (Security)
-- Risk: Search path injection attacks
-- Fix: Add "SET search_path = misir, pg_catalog, public" to all functions
-- ============================================================================

-- 1. Update updated_at trigger function
DROP FUNCTION IF EXISTS misir.update_updated_at() CASCADE;
...
-- 2. URL Normalization
DROP FUNCTION IF EXISTS misir.normalize_url(text) CASCADE;
...
-- 3. Domain Extraction
DROP FUNCTION IF EXISTS misir.extract_domain_from_url(text) CASCADE;
...
DROP FUNCTION IF EXISTS misir.set_normalized_url() CASCADE;
...
DROP FUNCTION IF EXISTS misir.set_domain_from_url() CASCADE;
...
DROP FUNCTION IF EXISTS misir.validate_reading_depth() CASCADE;
...
DROP FUNCTION IF EXISTS misir.insert_artifact_with_signal(uuid, bigint, bigint, bigint, text, text, text, vector, misir.engagement_level, misir.content_source, integer, double precision, double precision, integer, double precision, misir.signal_type, bigint[], timestamp with time zone) CASCADE;
CREATE OR REPLACE FUNCTION misir.insert_artifact_with_signal(
    ...
    -- Optional parameters (all must have defaults)
...
-- VERIFICATION
-- ============================================================================
-- Run this to verify the fix was applied successfully:
-- SELECT * FROM pg_proc WHERE proname IN (
--     'update_updated_at', 'normalize_url', 'extract_domain_from_url',
--     'set_normalized_url', 'set_domain_from_url', 'validate_reading_depth',
--     'update_subspace_centroid', 'insert_artifact_with_signal'
-- ) AND pronamespace = 'misir'::regnamespace;
-- 
-- Check that prosecdef column is NOT NULL (means SET search_path is set)
-- ============================================================================
